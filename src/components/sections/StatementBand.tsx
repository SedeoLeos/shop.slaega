...
export function StatementBand() {
  ...
  return (
    <section className="on-dark overflow-hidden bg-foreground py-10 text-foreground lg:py-14" aria-label="SLAEGA">
          ...
          <div key={copy} className="flex shrink-0 items-center" aria-hidden={copy === 1}>
            {words.map((word) => (
              <span key={word} className="type-display flex items-center whitespace-nowrap px-8">
                {word}
                <span className="ml-8 inline-block h-2 w-2 shrink-0 bg-spark" />
              </span>
            ))}
          </div>
        ))}
